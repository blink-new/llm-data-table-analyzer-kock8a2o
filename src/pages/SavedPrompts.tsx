...
const SavedPrompts: React.FC = () => {
  ...
  const handleUpdatePrompt = () => {
    if (!editingPrompt || !newPrompt.name.trim() || !newPrompt.prompt.trim()) return
    ...
    const updatedPrompt: SavedPrompt = {
      ...
      category: newPrompt.category,
      tags: newPrompt.tags.split(',').map(t => t.trim()).filter(Boolean),
      autoExecute: newPrompt.autoExecute,
      description: newPrompt.description,
      variables: newPrompt.variables ? 
        Object.fromEntries(newPrompt.variables.split(',').map(v => [v.trim(), ''])) : 
        undefined
    ...
    const updated = savedPrompts.map(p => p.id === editingPrompt.id ? updatedPrompt : p)
    savePrompts(updated)
  ...
  const handleUseTemplate = (template: PromptTemplate) => {
    ...
    savePrompts([...savedPrompts, prompt])
  }
